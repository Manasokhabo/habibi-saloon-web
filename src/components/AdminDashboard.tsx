...
const AdminDashboard: React.FC = () => {
  ...
  const [view, setView] = useState<'bookings' | 'leads' | 'customers' | 'hero' | 'reviews' | 'showcase' | 'transformations'>('bookings');
  ...
  const handleAddGalleryItem = async (type: 'showcase' | 'transformation') => {
    ...
    if (type === 'showcase' && !newGalleryItem.imageUrl) return alert("Select an image.");
    if (type === 'transformation' && (!newGalleryItem.beforeUrl || !newGalleryItem.afterUrl)) return alert("Select both images.");
    ...
    try {
      ...
      await firebaseService.addGalleryItem({ 
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6">
          ...
          <div className="flex glass p-1 rounded-xl border-white/10 overflow-x-auto no-scrollbar">
            {['bookings', 'leads', 'customers', 'hero', 'reviews', 'showcase', 'transformations'].map(tab => (
          ...
          <div className="animate-in fade-in duration-500">
            ...
            {view === 'leads' && (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {leads.length === 0 && <div className="col-span-full py-20 text-center opacity-20 uppercase text-[8px] tracking-[0.5em]">No Communication Logged</div>}
              </div>
            )}
            ...
            {view === 'customers' && (
              selectedCustomer ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  {users.length === 0 && <div className="col-span-full py-20 text-center opacity-20 uppercase text-[8px] tracking-[0.5em]">Registry Empty</div>}
                </div>
              )
            )}
            ...
            {view === 'hero' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  {heroImages.length === 0 && <div className="col-span-full py-20 text-center opacity-20 uppercase text-[8px] tracking-[0.5em]">No Visual Feed</div>}
                </div>
              </div>
            )}
            ...
            {view === 'reviews' && (
              <div className="space-y-6">
                   ...
                   <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <select value={newReview.rating} onChange={(e) => setNewReview({...newReview, rating: parseInt(e.target.value)})} className="bg-white/5 border border-white/10 rounded-lg p-2.5 text-white text-[9px] outline-none">
                           ...
                           <option value="1" className="bg-zinc-900">1 Star</option>
                        </select>
                      </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  {reviews.length === 0 && <div className="col-span-full py-20 text-center opacity-20 uppercase text-[8px] tracking-[0.5em]">No Testimonials Sync</div>}
                </div>
              </div>
            )}
          </div>
