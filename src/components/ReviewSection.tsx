...
const ReviewSection: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Priority: Real reviews first, then fallback to fill up to exactly 9 items
  ...
  // Chunk reviews based on responsive itemsPerPage
  ...
  return (
    <section className="py-8 px-6 relative overflow-hidden bg-[#050505]">
      <div className="max-w-7xl mx-auto">
        {/* Header with Navigation Indicator */}
        <div className="flex flex-col md:flex-row items-center justify-between mb-6 border-b border-white/5 pb-4">
        ...
        {/* Sliding Track - Successive Page Motion */}
        <div className="relative overflow-hidden h-[120px]">
          <div 
            className="flex transition-transform duration-700 cubic-bezier(0.4, 0, 0.2, 1) absolute inset-0"
            ...
            {paginatedReviews.map((group, groupIdx) => (
              <div key={groupIdx} className="w-full flex-shrink-0 grid grid-cols-1 md:grid-cols-3 gap-4">
                {group.map((rev) => (
                  <div 
                    key={rev.id} 
                    className="glass border border-white/10 rounded-xl p-3 flex items-center gap-4 hover:border-amber-500/20 transition-colors h-24"
                  >
                    {/* Customer Photo - Left (Original Color, Normal, No Grayscale) */}
                    <div className="shrink-0">
                      <div className="w-16 h-16 rounded-lg overflow-hidden border border-white/10 bg-zinc-900 shadow-lg">
                        <img 
                          src={rev.reviewImage || rev.avatar} 
                    ...
                    <div className="flex flex-col justify-center min-w-0">
                      ...
                      {/* Stars - Directly below text */}
                      <div className="flex gap-0.5">
                        {[...Array(5)].map((_, i) => (
                          <svg key={i} className={`w-2.5 h-2.5 ${i < rev.rating ? 'text-amber-500' : 'text-gray-800'}`} fill="currentColor" viewBox="0 0 20 20">
